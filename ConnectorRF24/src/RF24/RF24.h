...
public:
	...
	*/
	...
	void eventRF24();
	void registerCallback(void (*ptr)(char *data, uint8_t nLenght));
	bool isSending();
	void setIsSending(bool value);
	bool sendDataToAir(char *data);
...
private:
	...
	RF24(const RF24&);	//Prevent construction by copying
	RF24& operator=(const RF24&);	//Prevent assigment
	~RF24();					//Prevent unwanted destruction
	...
	*/
	...
	uint8_t readPayloadToCircularBuffer();
...
private:
	...
	//CCircularBuffer m_circBuffer;

	char spi_rxbuff[100] ; //SPI receive buffer (payload max 32 bytes)
	//uint8_t spi_txbuff[32+1] ; //SPI transmit buffer (payload max 32 bytes + 1 byte for the command)

	uint8_t m_nBuffHeader = 0;
	bool m_bNewLine = false;

	bool p_variant = false; /* False for RF24L01 and true for RF24L01P */
	uint8_t payload_size = MAXIMUM_PAYLOAD_SIZE; /**< Fixed size of payloads */
